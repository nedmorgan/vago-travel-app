...
export default class User extends Component {
  ...
  getUser = () => {
    axios
      .get(`/api/v1/users/${this.props.match.params.userId}`)
      .then(response => {
  ...
  updateUser = e => {
    e.preventDefault()
    axios
      .put(`/api/v1/users/${this.state.user._id}`, {
  ...
  deleteUser = () => {
    ...
    axios.delete(`/api/v1/users/${userId}`).then(res => {
  ...
  deleteCountry = (e, id) => {
    e.preventDefault()
    const userId = this.props.match.params.userId
    axios.delete(`/api/v1/users/${userId}/countries/${id}`).then(res => {
      this.getUser()
    })
  }
    ...
    return (
      <UserContainer>
        ...
        <button
          ...
          onClick={this.toggleUserEditForm}>
        ...
        <div className="user-info-flex">
          <h3>
            <a onClick={this.toggleCountriesList}>
              <i id="drp-icon-cty" class="drop-icon small material-icons left">
                arrow_drop_down
              </i>
            </a>
            Countries
            <a
              onClick={this.toggleCountryAddForm}
              className="add-country btn-floating btn-small waves-effect waves-light red">
              ...
              <div className="input-container row">
                ...
                <select
                  ...
                  {this.state.countryNames.map(name => {
                    ...
                    return (
                      <option value={name}>
                        {name.toUpperCase().replace(regex, ' ')}
                      </option>
                    )
                  })}
                </select>
              </div>
              <button className="add-country-button waves-effect waves-light btn">
                Add Country
              </button>
            </form>
          ) : null}
          ...
          <h3>
            <a onClick={this.toggleFavoritesList}>
              <i
                id="drp-icon-fav"
                className="drop-icon small material-icons left">
                arrow_drop_down
              </i>
            </a>
            Favorites
        ...
        <button
          className="delete-user waves-effect waves-light btn red"
